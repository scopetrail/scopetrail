...
async function mintFixture(issuerDid: string) {
...
describe('view-receipt CLI: verify from at:// URI (Sprint 03 Task 4)', () => {
  it('mock-published receipt: at:// URI -> stdout shows [ VERIFIED ], exit 0', async () => {
    ...
    try {
      ...
      writeFileSync(
        storePath,
        JSON.stringify([{ repo: issuerDid, collection: RECEIPT_NSID, rkey, record }])
      );
      ...
      assert.ok(
        res.stdout.includes(receipt.delegationContext.rootPrincipal.id),
        'expected root principal in output'
      );
    } finally {
  ...
  it('wrong collection in the URI -> exit 1, error printed, no crash', async () => {
    ...
    try {
      ...
      assert.ok(
        res.stderr.includes('WRONG_COLLECTION'),
        `expected WRONG_COLLECTION in stderr, got:\n${res.stderr}`
      );
  ...
  it('missing --jwks for an at:// URI -> fatal error, exit 1, no crash', () => {
    const res = spawnSync(
      process.execPath,
      [cliPath, 'at://did:plc:whatever/dev.scopetrail.auditReceipt/abc123'],
      { encoding: 'utf8', env: { ...process.env, ATP_MOCK: '1' } }
    );
