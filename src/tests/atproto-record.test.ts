...
import {
  ...
  atUriFor,
  rkeyFor,
} from '../atproto/record.js';
import type { AtpReceiptRecord } from '../atproto/record.js';
import { extractContext } from '../builder.js';
import { generateKeyPair, mintReceipt } from '../signer.js';
import type { OBOAuditReceipt, RawContextInput } from '../types.js';
...
describe('verifyMirrors', () => {
  ...
  it('returns false when the issuer mirror is edited', () => {
    const record = buildRecord(receipt);
    const tampered: AtpReceiptRecord = { ...record, issuer: 'did:web:attacker.example' };
    assert.equal(verifyMirrors(tampered), false);
  });

  it('returns false when the summary mirror is edited', () => {
    const record = buildRecord(receipt);
    const tampered: AtpReceiptRecord = { ...record, summary: 'Not the real summary' };
    assert.equal(verifyMirrors(tampered), false);
  });

  it('returns false when the schemaVersion mirror is edited', () => {
    const record = buildRecord(receipt);
    const tampered: AtpReceiptRecord = { ...record, schemaVersion: 'obo-receipt/v2' };
    assert.equal(verifyMirrors(tampered), false);
  });

  it('returns false (not throw) when receiptJson is unparsable', () => {
    const record = buildRecord(receipt);
    const tampered: AtpReceiptRecord = { ...record, receiptJson: 'not json at all' };
    assert.equal(verifyMirrors(tampered), false);
  });
});
