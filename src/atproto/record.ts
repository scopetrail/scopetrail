 ...
 */
export function rkeyFor(receipt: OBOAuditReceipt): string {
  if (!receipt.id.startsWith(RECEIPT_ID_PREFIX)) {
    throw new Error(
      `rkeyFor: receipt.id "${receipt.id}" does not start with expected prefix "${RECEIPT_ID_PREFIX}"`
    );
  }
 ...
 */
export function atUriFor(did: string, receipt: OBOAuditReceipt): string {
 ...
 */
export function buildRecord(receipt: OBOAuditReceipt): AtpReceiptRecord {
  return {
    $type: RECEIPT_NSID,
 ...
 */
export function parseRecord(record: AtpReceiptRecord): OBOAuditReceipt {
  return JSON.parse(record.receiptJson) as OBOAuditReceipt;
}
 ...
 */
export function verifyMirrors(record: AtpReceiptRecord): boolean {
  let receipt: OBOAuditReceipt;
  try {
