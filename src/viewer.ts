 ...
 */
export async function loadReceipt(source: string): Promise<OBOAuditReceipt> {
  ...
  try {
    ...
    throw new Error(`Not a valid OBOAuditReceipt — JSON parse failed: ${(e as Error).message}`);
  }
  ...
  return obj as unknown as OBOAuditReceipt;
}
...
export function renderChain(
  ...
  for (let i = 0; i < delegationChain.length; i++) {
    ...
    const expiryClause = hop.tokenRef.tokenExpiresAt
      ...
      : '';
    lines.push(
      `  │   token: ${shortDigest(hop.tokenRef.tokenDigest)}` +
