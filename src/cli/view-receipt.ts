...
interface CliArgs {
  ...
  mode: 'ascii' | 'markdown';
  jwksPath: string | null;  // path to local JWKS file
}

function parseArgs(argv: string[]): CliArgs {
  const args = argv.slice(2); // strip 'node' and script path
  let source = 'stdin';
  let mode: 'ascii' | 'markdown' = 'ascii';
  let jwksPath: string | null = null;
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--markdown') {
      ...
      if (!jwksPath) fatal('--jwks requires a file path argument');
 ...
 */
async function importPublicKeyFromJwks(
  jwksPath: string,
  verificationMethod: string
): Promise<CryptoKey | null> {
  let jwks: { keys: JsonWebKey[] };
  try {
    jwks = JSON.parse(readFileSync(jwksPath, 'utf8')) as { keys: JsonWebKey[] };
  } catch (e) {
    fatal(`Failed to read JWKS file "${jwksPath}": ${(e as Error).message}`);
  }
  ...
  const candidates = (jwks.keys as JwkWithKid[]).filter(
    k => k.kty === 'OKP' && k.crv === 'Ed25519' && (kidFromMethod == null || k.kid === kidFromMethod)
  );

  if (candidates.length === 0) {
    process.stderr.write(
      `Warning: no Ed25519 key matching kid "${kidFromMethod ?? 'any'}" found in JWKS. Trying first OKP key.\n`
    );
  ...
  try {
    ...
    process.stderr.write(`Warning: failed to import public key: ${(e as Error).message}\n`);
    return null;
  }
}
...
// ── at:// URI support (Sprint 03 Task 4) ─────────────────────────────────────

/** A seed entry written by a test into the file pointed to by `ATP_MOCK_STORE`. */
 ...
 */
async function buildAtpClient(): Promise<AtpClient> {
  if (process.env.ATP_MOCK === '1') {
    ...
    if (storePath) {
      let seed: MockStoreEntry[];
      try {
        seed = JSON.parse(readFileSync(storePath, 'utf8')) as MockStoreEntry[];
      } catch (e) {
        fatal(`Failed to read ATP_MOCK_STORE file "${storePath}": ${(e as Error).message}`);
      }
 ...
 */
async function runAtUriVerification(
  uri: string,
  ...
  try {
    jwks = JSON.parse(readFileSync(jwksPath, 'utf8')) as JsonWebKeySet;
  } catch (e) {
    fatal(`Failed to read JWKS file "${jwksPath}": ${(e as Error).message}`);
  }
...
main().catch(e => {
  process.stderr.write(`Unhandled error: ${(e as Error).message}\n`);
  process.exit(1);
});
