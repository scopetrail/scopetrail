...
import { generateKeyPair, mintReceipt, verifyReceipt, NonceStore, canonicalize, base58btcEncode, base58btcDecode, } from '../signer.js';
...
describe('JCS canonicalization', () => {
    ...
    it('sorts nested object keys', () => {
        const result = new TextDecoder().decode(canonicalize({ z: { y: 1, x: 2 } }));
        assert.equal(result, '{"z":{"x":2,"y":1}}');
    });
    it('preserves array order', () => {
        const result = new TextDecoder().decode(canonicalize({ arr: [3, 1, 2] }));
        assert.equal(result, '{"arr":[3,1,2]}');
    });
    it('serializes -0 as "0"', () => {
        const result = new TextDecoder().decode(canonicalize({ v: -0 }));
