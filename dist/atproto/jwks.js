 ...
 */
export async function fetchJwks(url, fetchFn = globalThis.fetch) {
    let res;
    try {
        res = await fetchFn(url);
    }
    catch (e) {
        throw new JwksFetchError(url, e.message);
    }
    if (!res.ok) {
    ...
    try {
        ...
        throw new JwksFetchError(url, `invalid JSON: ${e.message}`);
    }
    const jwks = body;
 ...
 */
export async function selectAndImportKey(jwks, kid) {
 ...
 */
export async function resolveKeyFromJwksUrl(url, verificationMethod, fetchFn = globalThis.fetch) {
