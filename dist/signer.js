 ...
 */
export async function hashDocument(canonical) {
...
export function buildSummary(context) {
    const name = (p) => p.displayName ?? p.id;
    return (`${name(context.rootPrincipal)} authorized ${name(context.actingPrincipal)} ` +
 ...
 */
export async function mintReceipt(context, privateKey, keyId, issuerDid) {
    ...
    const sigBuffer = await subtle.sign('Ed25519', privateKey, canonical);
 ...
 */
export async function verifyReceipt(receipt, publicKey, nonceStore = defaultNonceStore) {
    ...
    const chainResult = validateChain(receipt.delegationContext.rootPrincipal, receipt.delegationContext.delegationChain, receipt.credentialSubject, receipt.delegationContext.grantedScopes);
...
export async function buildJwks(keys) {
    const jwkKeys = await Promise.all(keys.map(async ({ kid, publicKey }) => {
