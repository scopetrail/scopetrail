 ...
 */
export function digestToken(rawToken) {
 ...
 */
export function extractJwtClaims(jwt) {
 ...
 */
export function sortChain(hops) {
 ...
 */
export function validateChain(root, chain, actor, grantedScopes) {
    const errors = [];
...
function buildTokenRef(rawToken, tokenType, fieldPrefix, errors) {
    if (tokenType === 'jwt') {
        try {
            ...
            return {
                ...
                tokenExpiresAt: typeof claims.exp === 'number'
...
export function extractContext(input) {
    ...
    const hopsWithRefs = input.hops.map((hop, i) => {
    ...
    const chainResult = validateChain(input.rootPrincipal, sortedChain, input.actingPrincipal, input.grantedScopes);
    errors.push(...chainResult.errors);
    // Build upstream TokenRef
    const upstreamTokenRef = buildTokenRef(input.rawUpstreamToken, input.upstreamTokenType, 'rawUpstreamToken', errors);
    if (errors.length > 0) {
